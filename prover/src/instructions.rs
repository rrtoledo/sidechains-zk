...
/// `Assigned` is a witness that is already assigned in a previous level and is about to be copy
/// constrained at current level. `Unassigned` is a new witness value which will be
/// turned into a `AssignedValue`. `Zero` should be used for unused cells at the
/// current row. Non zero terms have a coupled field element which is a
/// multiplicative factor of the term and will be assigned as fixed value in
/// synthesis time. This can be interpreted as the value of their selector polynomial. Note that
/// if, for example, we want to include a multiplication gate, the coupled field element will be
/// zero and, instead, the selector of multiplication will be one. For example in and addition
/// circuit:
...
impl<'a, F: PrimeField> Term<'a, F> {
    ...
    /// Returns the witness part of this term
    ...
    /// Returns the fixed part of this term
    pub fn base(&self) -> F {
        match self {
    ...
    /// Composes terms as
    /// `w_0 * s_0 + w_1 * s_1 + ...`
    /// And returns the calculated witness
...
/// `CombinationOptionCommon` defines the behaviour of the `main_gate` at the
/// current row. Options here can be applied to most of the standard-like circuits
/// as long as they have one multiplication gate, one addition gate, and one further
/// rotation gate.
#[derive(Clone, Debug)]
pub enum CombinationOptionCommon<F: PrimeField> {
    ...
    /// All multiplication gates are closed and combines terms to the next
    /// row
    CombineToNextAdd(F),
}

/// `MainGateInstructions` covers many basic constraints such as assignments, logical and
/// arithmetic operations. It also includes general purpose `combine` and `apply`
/// functions to let user build custom constraints using this main gate
pub trait MainGateInstructions<F: PrimeField, const WIDTH: usize>: Chip<F> {
    ...
    /// Assigns a tuple of witnesses to their corresponding columns by taking the
    /// index `i` of its position, and placing the witness in column `i % 5`.
    fn assign_values_slice(
        &self,
        ctx: &mut RegionCtx<'_, F>,
        unassigned: &[Value<F>],
        ...
        unassigned
            ...
            .enumerate()
            .map(|(index, &value)| {
                self.assign_to_column(ctx, value, Self::MainGateColumn::from_index(index as u8))
            })
            .collect()
    }
    ...
    // `new_assigned_value + c0 * c1 - c0 - c1 = 0`.
    fn or(
        &self,
        ctx: &mut RegionCtx<'_, F>,
    ...
    /// unsafe because if witness `b` is zero it cannot find a valid witness.
    /// In such a case, it returns a witness which is equal to zero.
    fn div_unsafe(
        &self,
        ctx: &mut RegionCtx<'_, F>,
        a: &AssignedValue<F>,
    ...
    /// unsafe because if the witness equals to zero a valid witness cannot be
    /// found, and zero is returned instead.
    fn invert_unsafe(
        &self,
        ctx: &mut RegionCtx<'_, F>,
        a: &AssignedValue<F>,
    ...
    /// Assigns new witness that should be equal to `1/a`. We want to cover the case
    /// where `a` is non invertible for which we create a `cond` flag which is assigned
    /// to `0` otherwise `1`.
    fn invert(
        &self,
        ctx: &mut RegionCtx<'_, F>,
        ...
        // Returns 'r' as a condition bit that defines if inversion was successful or not
    ...
    /// Enforces an assigned value to be equal to a fixed value.
    ...
    /// Enforces two witnesses are not equal
    fn assert_not_equal(
        &self,
        ctx: &mut RegionCtx<'_, F>,
        a: &AssignedValue<F>,
    ...
    /// Assigns new value `r` to 1 if two values are equal and 0 otherwise
    fn is_equal(
        &self,
        ctx: &mut RegionCtx<'_, F>,
        a: &AssignedValue<F>,
    ...
    /// Assigns new bit flag `1` if given value equals to `1` otherwise assigns
    /// `0`
    ...
    /// Assigns a new bit witness `r` to `0` if both given witnesses are not `0`
    /// otherwise `1`
    fn nand(
        &self,
        ctx: &mut RegionCtx<'_, F>,
    ...
    /// Assigns new witness that equals to `a` if `cond` is true or assigned to
    /// `b` if `cond is false
    fn select(
        &self,
        ctx: &mut RegionCtx<'_, F>,
    ...
    /// Assigns new witness that equals to `to_be_selected` if `cond` is true or
    /// assigned to `to_be_assigned` which is a constant if `cond is false
    fn select_or_assign(
        &self,
        ctx: &mut RegionCtx<'_, F>,
